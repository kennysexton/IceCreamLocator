...
public class DbMods {
    ...
    public static StringData update(StringData inputData, DbConn dbc) {
        ...
        if (errorMsgs.getCharacterCount() > 0) {  // at least one field has an error, don't go any further.
            ...
            String sql = "UPDATE supply SET selling_price=?, special_additions=?, "+
                    "flavor_id=?, web_user_id=? WHERE supply_id = ?";
            ...
            // Encode string values into the prepared statement (wrapper class).
            pStatement.setBigDecimal(1, ValidationUtils.decimalConversion(inputData.selling_price));
            pStatement.setString(2, inputData.special_additions); // string type is simple
            pStatement.setInt(3, ValidationUtils.integerConversion(inputData.flavor_id));
            pStatement.setInt(4, ValidationUtils.integerConversion(inputData.web_user_id));
            pStatement.setInt(5, ValidationUtils.integerConversion(inputData.supply_id));
            ...
            if (errorMsgs.errorMsg.length() == 0) {
                ...
                errorMsgs.errorMsg = "Invalid supply_Id";
            } else if (errorMsgs.errorMsg.contains("Duplicate entry")) {
                errorMsgs.errorMsg = "That supply is already taken";
            }
